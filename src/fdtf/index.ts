...
type FlexiDate = string | Moment
type M1Format = 'm1a' | 'm1b' | 'm1c'
type M2Format = 'm2a' | 'm2b' | 'm2c' | 'm2d' | 'm2e'
type M3Format = 'm3a' | 'm3b'
type K1Format = 'k1a' | 'k1b'
type K2Format = 'k21a' | 'k21b' | 'k22a' | 'k22b'

export const momentDateToString = (date: FlexiDate = moment(), format = 'YYYY-MM-DD') => {
  return moment(moment(date), format).format(format)
}

const parseDT = (dt: FlexiDate | null = null, format: string) => parseInt(momentDateToString(dt ? moment(dt) : moment(), format))

const sameYear = (dt1: FlexiDate, dt2: FlexiDate = moment()) => {
  const s1 = parseDT(dt1, 'YYYY')
  const s2 = parseDT(dt2, 'YYYY')
  return s1 === s2
}

const addDateTime = (num = 1, time: any = 'd', format = 'DD/MM/YYYY') => momentDateToString(moment().add(num, time), format)

const subtractDateTime = (num = 1, time: any = 'd', format = 'DD/MM/YYYY') => momentDateToString(moment().subtract(num, time), format)

const getWeekDay = (dt: FlexiDate = moment()) => {
  const getDay = moment(dt).day()
  return getDay === 0 ? 'Chủ nhật' : `Thứ ${getDay + 1}`
}

export const dateTime = (endTime: FlexiDate = moment(), startTime: FlexiDate = moment(), isPast = false, isAbs = false) => {
...
export const fdtfM3Plus = (dt: FlexiDate, format = `HH[h]mm’ss’’ DD/MM`) => {
  if (sameYear(dt)) return momentDateToString(dt, format)
  return momentDateToString(dt, `${format}/YYYY`)
}

export const fdtfM1 = (dt: FlexiDate, mx: M1Format = 'm1b') => {
  let fm = ''
  if (mx === 'm1a') {
    fm = `HH[h]`
    if (moment(dt)?.hour() < 10) return momentDateToString(dt, `H[h]`)
  }
  if (mx === 'm1b') fm = `HH[h]mm[’]`
  if (mx === 'm1c') fm = `HH[h]mm[’]ss[’’]`
  return momentDateToString(dt, `${fm}`)
}

export const fdtfM2 = (dt: FlexiDate, mx: M2Format = 'm2b') => {
  let fm = `HH[h]mm[']`
  if (mx === 'm2a') fm = `YYYY`
  if (mx === 'm2b') fm = `MM/YYYY`
  if (mx === 'm2c') fm = `DD/MM`
  if (mx === 'm2d') fm = `DD/MM/YYYY`
  if (mx === 'm2e') fm = sameYear(dt) ? `DD/MM` : `DD/MM/YYYY`
  return momentDateToString(dt, fm)
}

export const fdtfM3 = (dt: FlexiDate, mx: M3Format = 'm3b') => {
  if (mx === 'm3a') return `${fdtfM1(dt, 'm1b')} ${fdtfM2(dt, 'm2e')}`
  return `${fdtfM1(dt, 'm1c')} ${fdtfM2(dt, 'm2e')}`
}

export const fdtfM4 = (dt: FlexiDate, checkReturn = false) => {
...
export const fdtfM6 = (dt: FlexiDate, checkReturn = false) => {
...
export const fdtfM7 = (dt: FlexiDate) => {
  return momentDateToString(dt, `[${getWeekDay(dt)}, ngày] DD [tháng] MM [năm] YYYY`)
}

export const fdtfM8 = (dt: FlexiDate) => {
  ...
  if (fdtfM5(dt, true)) return `${getWeekDay(dt)} ${fdtfM5(dt, true)} (${getDay})`
...
export const fdtfK1 = (dt1: FlexiDate, dt2: FlexiDate, kx: K1Format = 'k1b') => {
  ...
  if (timeLeft < 60 * 60000) return `${minutes} phút${!isC1 && seconds !== 0 ? ` ${seconds} giây` : ''}`
  if (timeLeft < 24 * 60 * 60000) return `${hours} giờ${!isC1 && minutes !== 0 ? ` ${minutes} phút` : ''}`
  if (asHours >= 24 && asDays < 7) return `${asDays} ngày${!isC1 && asHours % 24 !== 0 ? ` ${asHours % 24} giờ` : ''}`

  if (7 <= asDays && months === 0) return `${Math.trunc(days / 7)} tuần${!isC1 && days % 7 !== 0 ? ` ${days % 7} ngày` : ''}`
  if (0 < months && months < 12) return `${months} tháng${!isC1 && days !== 0 ? ` ${days} ngày` : ''}`
